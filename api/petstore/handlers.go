...
// Here, we implement all the handlers in the ServerInterface

func (p *Handlers) FindPets(ctx echo.Context, params FindPetsParams) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()
...
func (p *Handlers) AddPet(ctx echo.Context) error {
...
func (p *Handlers) FindPetByID(ctx echo.Context, petId int64) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()
...
func (p *Handlers) DeletePet(ctx echo.Context, id int64) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()
